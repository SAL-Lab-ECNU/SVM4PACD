...
# Function to generate dependent data
def generate_dependence_data(IMP,Qm,I,N,sg):
    Qm_sampled = Qm.sample(n = I,replace = True,random_state = seed).reset_index(drop=True)
    alpha = IMP.sample(n = N,replace = True,random_state = seed).reset_index(drop=True)
    guess = np.full((I,),sg)
    slip = np.full((I,),sg)
    dat = pd.DataFrame(np.zeros((N,I)))
    latresp = pd.DataFrame(np.zeros((N,I)))
    for item in range(I):
        latresp.iloc[:,item] = (alpha.ge(Qm_sampled.iloc[item, :], axis=1).mean(axis=1) == 1).astype(int)
        prob = np.where(latresp.iloc[:, item] == 1, 1 - slip[item], guess[item])
        dat.iloc[:, item] = (np.random.rand(N) < prob).astype(int)
    datb = pd.concat([dat,alpha],axis=1)
...
def train_model_twostep(X_train,y_train,X_test,y_test,attri_num):
    ...
    for j in range(attri_num):
        ...
        # Determine the number of splits based on the smallest class count
        min_class_count = y_train.iloc[:,j].value_counts().min()
...
if __name__ == '__main__':
    ...
    min_count = 2   #minimum number of samples in each class
    cycles = 1       #number of iterations: 30
    ah_name_list = ('linear','divergent','convergent')      #attribute hierarchy name list
    ah_name = ah_name_list[ah]                        #attribute hierarchy name
    ...
    for i in range(cycles):
        datb = generate_dependence_data(imp,Qm,Item,N,sg)
